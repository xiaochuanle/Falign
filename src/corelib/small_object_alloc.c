...
static const u32 kMemChunkSize = 8 * 1024;
static const int kDeallocatedObjectListSize = 100;
...
SmallObjectAllocNew(const u32 object_size)
{
    ...
    alloc->deallocated_object_list = (void**)calloc(kDeallocatedObjectListSize, sizeof(void*));
    alloc->deallocated_objects = 0;
	alloc->num_alloc = 0;
    return alloc;
}
...
SmallObjectAllocFree(SmallObjectAlloc* alloc)
{
    ...
    free(alloc->deallocated_object_list);
    free(alloc);
    return NULL;
}
...
{
    if (alloc->deallocated_objects) {
        alloc->deallocated_objects--;
        return alloc->deallocated_object_list[alloc->deallocated_objects];
    }

    return SmallObjectAllocAlloc(alloc, 1);
}

void
SmallocObjactAllocDeallocOne(SmallObjectAlloc* alloc, void* object)
{
    if (alloc->deallocated_objects < kDeallocatedObjectListSize) {
        alloc->deallocated_object_list[alloc->deallocated_objects] = object;
        alloc->deallocated_objects++;
    }
}
