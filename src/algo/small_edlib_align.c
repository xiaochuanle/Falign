...
SmallEdlibAlignResult*
SmallEdlibAlignResultFree(SmallEdlibAlignResult* result)
{
...
void
SmallEdlibAlignResultClear(SmallEdlibAlignResult* result)
{
...
SmallEdlibAlignData*
SmallEdlibAlignDataNew(int maxNumBlocks, int targetLength)
{
    SmallEdlibAlignData* data = (SmallEdlibAlignData*)malloc(sizeof(SmallEdlibAlignData));
    data->align_matrix = EdlibAlignMatrixNew(maxNumBlocks, targetLength);
    data->result = SmallEdlibAlignResultNew();
...
SmallEdlibAlignData*
SmallEdlibAlignDataFree(SmallEdlibAlignData* data)
{
    data->align_matrix = EdlibAlignMatrixFree(data->align_matrix);
    data->result = SmallEdlibAlignResultFree(data->result);
    free(data->blocks);
	free(data->peq);
...
{
    SmallEdlibAlignData* data = SmallEdlibAlignDataNew(MaxNumBlocks, MaxSeqSize);
    return (small_edlib_align_struct*)(data);
}
...
small_edlib_align_struct_free(small_edlib_align_struct* data)
{
    ...
    SmallEdlibAlignDataFree(x_data);
    return NULL;
}
        ...
        kstring_t* saln)
{
	...
	SmallEdlibAlignResultClear(align_data->result);
	...
	if (edit_distance == -1) return -1;
    hbn_assert(edit_distance >= 0, "dist = %d, qsize = %d, ssize = %d, end_position = %d", edit_distance,
		query_size, target_size, end_position);
